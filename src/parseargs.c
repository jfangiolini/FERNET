...
struct args parseArgs(int argc, char **argv)
{
	...
	arg_file1(NULL, NULL, "<input>", "input position file");
    struct arg_file *config =
	arg_file1("c", "config", "<config file>", "configuration file");
    struct arg_lit *help =
	...
	printf
	    ("FERNET: Fluorescence Emission Recipes and NumErical routines Network.\n");
	printf
	    ("This program evaluates the emision of photons in different fluorescence experiments.\n");
	printf
	    ("The input file must have the positions of each molecule in each time step.\n");
