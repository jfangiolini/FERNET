 ...
 ***********************************************************************************/
int lineRoutine(config_t cfg, const char *filename)
{
	...
	centros[i] =
	    i * lParms.shift - (lParms.ncolumn - 1) * lParms.shift / 2;
    }
		...
		for (int i = 0; i < cParms.sChannel[0].nmols; i++) {
			...
			nphot[0] +=
			    gaussPSF(x, y, z, cParms.w_xy, cParms.w_z,
				     centros[column] - lParms.centerx,
				     lParms.centery, lParms.centerz,
				     cParms.nevents,
