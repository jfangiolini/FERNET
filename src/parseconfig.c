...
struct commonParms parseCommon(config_t cfg, FILE * fileIn)
{
	...
	/* Parse channel 0 */
	config_setting_t *pChann0 = config_setting_get_member(common, "channel0");
	if (pChann0 == NULL) {
	...
	/* Parse channel 1 */
	config_setting_t *pChann1 = config_setting_get_member(common, "channel1");
	if (pChann0 == NULL) {
	...
	/* Get noise status */
	if (!config_setting_lookup_int(common, "noise_on", &cParms.noise)) {
		parseError("noise_on");
	}
...
struct stackParms parseStack(config_t cfg)
{
	...
	if (sParms.bot_z > sParms.top_z) {
		fprintf(stderr, "Error: bot_z position is greater than top_z position.\n");
		/* cerrar files? */
		exit(1);
	}
...
struct orbitParms parseOrbit(config_t cfg)
{
	...
	/* Get output name */
	if (!config_setting_lookup_string(orbit, "tiffname", &orParms.tiffname)) {
		parseError("tiffname");
	}
...
struct channelInfo parseChannel(config_setting_t * pChan, float dt)
{
	...
	if (chan.status == 1) {
		const config_setting_t *molnames = config_setting_get_member(pChan, "molec");
		if (molnames == NULL) {
			parseError("molec");
		} else {
			...
			for (int i = 0; i < chan.nmols; i++) {
				...
				chan.mols[i] = (char *)malloc((strlen(tmp) + 1) * sizeof(char));
				strcpy(chan.mols[i], tmp);
			}

			const config_setting_t *molbright = config_setting_get_member(pChan, "bright");
			if (config_setting_length(molnames) != config_setting_length(molbright)) {
				fprintf(stderr, "Molecules name and brightness list must be equal in size.\n");
				exit(1);
			}
