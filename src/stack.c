...
***********************************************************************************/
int stackRoutine(config_t cfg, const char *filename, gsl_rng * r)
{
	...
	for (int i = 0; i < sParms.width; i++) {
		centerx[i] = i * sParms.pixel - (sParms.width - 1) * sParms.pixel / 2;
	}

	for (int i = 0; i < sParms.height; i++) {
		centery[i] = i * sParms.pixel - (sParms.height - 1) * sParms.pixel / 2;
	}
					...
					if (!strcmp(molname, cParms.sChannel[0].mols[i])) {
						nphot[0] +=
						    gaussPSF(x, y, z,
							     ...
							     centerx[column],
							     centery[row],
							     zpos[slice], cParms.nevents, cParms.sChannel[0].q[i], r);
					}
				}
			}
					...
					if (!strcmp(molname, cParms.sChannel[1].mols[i])) {
						nphot[1] +=
						    gaussPSF(x, y, z,
							     ...
							     centerx[column],
							     centery[row],
							     zpos[slice], cParms.nevents, cParms.sChannel[1].q[i], r);
					}
				}
			}
		}
